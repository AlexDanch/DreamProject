...
sub setUpPostRequest()
   ...
   m._urlRequest = CreateObject("roSgNode", "UrlRequest")
   m._urlRequest.observeField("response", "onLoginResponseRecived")
   m._urlRequest.url = "https://2cd1f0e1-de20-47d8-b992-8f58a80f4e11.mock.pstmn.io/login"
   m._urlRequest.body =  m.requstObj
   m._urlRequest.control = "run"
end sub

sub onLoginResponseRecived(event)
      loginResponseRecieve = event.getData()
      ? "loginResponseRecieve" loginResponseRecieve
end sub
